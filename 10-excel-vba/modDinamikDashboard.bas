...
Private Sub SabitleriKur()
    DASH = "DİNAMİK DASHBOARD"
    VTAB = "VERİ TABANI"
    STAB = "DASH AYAR"
    OZET = "ÖZET VERİ"
    SRCSH = "STOK HAREKETLERİ"
    MLIST = "MALZEME LİSTESİ"
    TUMU = "TÜMÜ"
    GIRISK = "GİRİŞ"
    CIKISK = "ÇIKIŞ"
    IADEK = "İADE"
...
Private Sub AyarSayfasiKur()
    ...
    wa.Range("A1").Value = "AKTİF FİLTRE (ARA butonu doldurur - elle değiştirmeyin)"
    Dim lbl As Variant
    lbl = Array("AY", "MALZEME TÜRÜ", "SINIF", "KULLANIM YERİ", "MALZEME ADI")
    ...
    wa.Range("M1").Value = "SINIF SEÇENEKLERİ"
    wa.Range("N1").Value = "MALZEME ADI SEÇENEKLERİ"
    wa.Range("P1").Value = "TÜR SEÇENEKLERİ"
    wa.Range("Q1").Value = "YER SEÇENEKLERİ"
    wa.Range("R1").Value = "AY SEÇENEKLERİ"
...
Private Sub DashboardKur()
    ...
    With ws.Range("A1:L2")
        .Merge
        .Value = "TCH STOK YÖNETİMİ  •  DİNAMİK DASHBOARD"
    ...
    ws.Range("A5:B5").Merge: ws.Range("A5").Value = "KULLANIM YERİ"
    ...
    ws.Range("A8").Value = "TOPLAM GİRİŞ"
    ws.Range("C8").Value = "TOPLAM ÇIKIŞ"
    ws.Range("E8").Value = "HAREKET SAYISI"
    ws.Range("G8").Value = "NET (GİRİŞ-ÇIKIŞ)"
    ws.Range("I8").Value = "SEÇİLEN KRİTER"
    ...
    ws.Range("A10").Value = "Kullanım:  Açılır listelerden filtreleri seçin, ARA'ya basın. Üst kriter (Malzeme Türü, Sınıf, Malzeme Adı) değişince alt listeler otomatik daralır. TEMİZLE filtreleri sıfırlar. YENİLE stok hareketlerini yeniden okur."
    ...
    ws.Range("A12").Value = "SEÇİME GÖRE FİLTRELENEN HAREKETLER"
    ...
    Dim th As Variant
    th = Array("AY", "İŞLEM TÜRÜ", "MALZEME ADI", "PLAKA / AÇIKLAMA", "MİKTAR", "PERSONEL", "TARİH", "MALZEME TÜRÜ", "SINIF", "KULLANIM YERİ", "GİRİŞ", "ÇIKIŞ")
...
Private Sub OzetSayfasiKur()
    ...
    oz.Cells.Clear
    oz.Range("A1").Value = "ÖZET VERİ (otomatik güncellenir)"
    oz.Range("A1").Font.Size = 16: oz.Range("A1").Font.Bold = True: oz.Range("A1").Font.Color = RGB(31, 60, 95)
    oz.Range("A3").Value = "AYLIK GİRİŞ / ÇIKIŞ"
    oz.Range("A4").Value = "AY": oz.Range("B4").Value = GIRISK: oz.Range("C4").Value = CIKISK
    oz.Range("E3").Value = "MALZEME TÜRÜNE GÖRE ÇIKIŞ"
    oz.Range("E4").Value = "MALZEME TÜRÜ": oz.Range("F4").Value = CIKISK
    oz.Range("A20").Value = "SINIFA GÖRE ÇIKIŞ"
    oz.Range("A21").Value = "SINIF": oz.Range("B21").Value = CIKISK
    oz.Range("E20").Value = "KULLANIM YERİNE GÖRE ÇIKIŞ"
    oz.Range("E21").Value = "KULLANIM YERİ": oz.Range("F21").Value = CIKISK
...
Private Sub BasliklariYazVTAB(vt As Worksheet)
    Dim h As Variant
    h = Array("AY", "İŞLEM TÜRÜ", "MALZEME ADI", "MİKTAR", "PERSONEL", "TARİH", "MALZEME TÜRÜ", "SINIF", "KULLANIM YERİ", "GİRİŞ", "ÇIKIŞ", "PLAKA / AÇIKLAMA")
    Dim i As Long
...
Public Sub AraButonu()
    ...
    If son >= 2 Then
        ...
        Else
            ws.Range("A14").Value = "Seçilen kritere uygun kayıt bulunamadı."
        End If
...
ArayipGoster:
    ...
    If sonucSayisi = 0 Then
        ...
        yeniArama = InputBox("'" & aramaTerimi & "' içeren malzeme bulunamadı." & vbCrLf & vbCrLf & _
                             "Farklı bir kelime yaz" & vbCrLf & "0 = İPTAL (TÜMÜ'ye döner)", "Malzeme Bulunamadı")
    ...
    mesaj = mesaj & vbCrLf & "Seç (1-" & gosterSayisi & ")" & vbCrLf & _
            "  0 = İPTAL (TÜMÜ'ye döner)" & vbCrLf & "  Başka kelime yazarsan tekrar arar"
...
Public Sub GrafikleriYap()
    ...
    With ch1.Chart
        ...
        End With
        .HasTitle = True: .ChartTitle.Text = "AYLIK GİRİŞ / ÇIKIŞ"
        .HasLegend = True
    End With
    ...
    With ch2.Chart
        ...
        End With
        .HasTitle = True: .ChartTitle.Text = "SINIFA GÖRE ÇIKIŞ (İLK 10)"
        .HasLegend = False
    End With
    ...
    With ch3.Chart
        ...
        End With
        .HasTitle = True: .ChartTitle.Text = "AYLIK GİRİŞ / ÇIKIŞ (ÖZET)"
        .HasLegend = True
    End With
    Application.ScreenUpdating = True
End Sub
...
Public Sub ButonlariYap()
    ...
    ButonYap ws, ws.Range("J5:K6"), "btnTemizle", "TEMİZLE", RGB(245, 124, 0), "TemizleButonu"
    ButonYap ws, ws.Range("J7:K8"), "btnYenile", "YENİLE", RGB(33, 110, 180), "DashYenile"
    On Error Resume Next
